...
class TaskAdapter(MetaTemplate):
    ...
    def forward(self, x, label):
        ...
        # P2: cos_score 由 cosine_similarity(enh_embedding[i], q_aft_tm[...])
        # 得到，朝向为 [n_way, N_Q]；视觉分为 [N_Q, n_way]。转置语义分使
        # 两分支同为 (查询行, 类列)，消除逐元素相乘时的行列语义混杂
        # （n_query=1 且查询按类序排列时 argmax 大概率仍落对，掩盖了该
        # 问题；修复后基线相对论文移动属预期，一切对比以修复后 B0 为准）。
        sem_dists = cos_score.transpose(-2, -1)
        vis_dists = cosine_similarity(z_query.mean(1), z_proto.mean(1)).squeeze()
        ...
        # P1: 返回顺序改为 (视觉分, 语义分)，与 run.py 三处接收点
        # `vis_dis, sem_dis = model(...)` 的命名一致（此前返回
        # (sem_dists, dists)，语义分被当作视觉分接收；融合乘积可交换、
        # 终分不变，但分支级分析会张冠李戴）。
