...
if __name__=='__main__':
    ...
    setup_seed(getattr(params, 'seed', 916)) #随机种子（config 可配 seed，缺省 916，行为不变）
    ...
    if params.method in ['taskadapter']:
        ...
        num_workers = getattr(params, 'num_workers', 8)  # 坑 D：config 可配

        train_few_shot_params    = dict(n_way = params.train_n_way, n_support = params.n_shot)
        base_datamgr            = SetDataManager(image_size, n_query = n_query,  num_segments = params.num_segments, n_eposide=params.train_episode, num_workers=num_workers, **train_few_shot_params)
        base_loader             = base_datamgr.get_data_loader( base_file , aug = params.train_aug )

        test_few_shot_params     = dict(n_way = params.test_n_way, n_support = params.n_shot)
        val_datamgr             = SetDataManager(image_size, n_query = n_query,  num_segments = params.num_segments, n_eposide = params.eval_episode, num_workers=num_workers, **test_few_shot_params)
        val_loader              = val_datamgr.get_data_loader( val_file, aug = False)

        test_datamgr             = SetDataManager(image_size, n_query = n_query,  num_segments = params.num_segments, n_eposide = params.test_episode, num_workers=num_workers, **test_few_shot_params)
