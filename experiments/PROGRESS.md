# PROGRESS — 跨会话进度
最后更新：2026-09-05（拍板：步 B 前置只推理 λ 探针 → 交接包⑫已发；判据修订为"数值健康+内容贡献"双门槛；⑩ 门控于 ε=0.1 探针通过）
...
| M8 | 创新点 1 步 B（λ 扫描，λ-Acc/λ-OS 双曲线） | ◐ | ⑪ 熵判据未过（ε=0.1 π熵0.999 趋均匀，但此为 λ=0 平衡OT 的 §1.2 预期机制、非故障）→ 拍板先发**只推理 λ 探针⑫**（内容贡献门控）；⑩ 门控于 ε=0.1 探针四判据全过（band↑/π_full≠π_pos/plan 随内容变化/条件熵↓） |
...
## 云端待执行（交接包队列）
- **交接包⑫ M8pre-I1-probe（已发，待 Cursor 执行）**：`handoff/2026-09-05_M8pre-I1-lambda-probe.md`，代码 `6d742a2`。只推理，用步A 三档 ckpt（ε=0.01/0.05/0.1，不重训）在同 1000 配对 episode 扫 λ∈{0,0.1,0.3,1,3}，记 row_cond_entropy/band_mass/content_l1(π_full vs π_pos)/cross_class_l1/cross_query_l1 + aux acc/OS。判据主看 ε=0.1：band↑ ∧ 中等λ π_full≠π_pos ∧ plan 随类/query 变化 ∧ 条件熵↓。~1.7GB、~25–30min/档、三档链~1.5h（待实测）。GPU **空闲**。
- **交接包⑩ M8-I1B（门控中，未启动）**：`handoff/2026-09-05_M8-innov1-stepB.md`。**放行前置＝⑫ 中 ε=0.1 四判据全过**（内容自适应，非纯几何软窗口）；过则固定 ε*=0.1 扫 λ∈{0.1,0.3,1,3}，否则按⑫放行逻辑改选更小 ε 或回报。
...
4. **待收包⑫**：判 ε=0.1 是否内容自适应（band↑ ∧ π_full≠π_pos ∧ plan 随内容变化 ∧ 条件熵↓）→ 过则固定 ε*=0.1 放行步 B⑩，否则比较更小 ε 内容贡献再选。
5. 后续：步 C（ρ 不平衡+时序鲁棒截断）→ M9 消融终表（10000ep 核心行 ①B0/②OT平衡/③+λ/④+ρ）；创新点 2/4a 手册已入库待创新点 1 主线定档。
