# PROGRESS — 跨会话进度
最后更新：2026-09-06（⑩/⑫ 已收包；用户冻结 λ*=0.3；步 C 交接包⑬已发：扫 ρ + 阶段D 时序鲁棒截断评测，代码 `149f555`）
...
| M9 | 创新点 1 步 C（ρ 不平衡+时序鲁棒）+ 消融终表 10000ep | ◐ | **λ*=0.3 用户冻结**；步 C 交接包⑬已发（ε0.1/λ0.3 扫 ρ∈{None,10,1,0.1} + 四窗截断评测 + B0/λ0 对照，代码 `149f555`）；微网格与 10000ep 终表待 ρ 收包 |
...
## 云端待执行（交接包队列）
- **交接包⑬ M9pre-I1-stepC（已发，待 Cursor 执行）**：`handoff/2026-09-06_M9pre-I1-stepC-rho-robust.md`，代码 `149f555`。固定 ε0.1/λ0.3，扫 ρ∈{None,10,1,0.1}（None=复用 I1B_lam03 不重训）；每档 3a 诊断(OS)+四窗时序鲁棒截断评测(正常/掐头/去尾/收缩)；另 B0(固定窗口)+λ0(平衡OT)补同口径截断对照。**λ 冻结 0.3**（不用 10000ep test 重选）。~11h（训练7.2h+评测4h）。GPU **空闲**。
- 已收并登账：⑩ M8-I1B（λ 扫描，λ*=0.3）、⑫ M8pre-I1-probe（门控PASS）、⑪ M7-I1A-recheck、⑨ M7-I1A、⑧ M6-3b、⑦ M5-3b、⑥ M4-3a-final、⑤ M3-3a-dev、④ seed2more（B0=57.01±1.64）、③ multiseed、② 历史单跑。
- **当前连云**：Host `gpuhome` → `sc01-ssh.gpuhome.cc` **Port 30448**。GPU **空闲**。

## 已回收待登账
-（空；⑩ 四档 λ 已入 ledger。建议 λ*=0.3）
...
6. **✅ 步 C 交接包⑬已发（本会话）**：λ*=0.3 冻结，扫 ρ∈{None,10,1,0.1}+四窗截断评测+B0/λ0 对照，代码 `149f555`（阶段D 截断采样 + robust_eval，单测 162/162）。待 Cursor 执行。
7. **待收包⑬后**：定 ρ*（主看截断 ΔAcc）→ 2×2 λ×ρ 微网格复核 → 全冻后 M9 消融终表（10000ep ①B0/②OT平衡/③+λ*/④+ρ* + 1 种子复跑）；创新点 2/4a 手册已入库待创新点 1 主线定档。
