# PROGRESS — 跨会话进度
最后更新：2026-09-07（⑬ 已收；机制诊断 ρ=10≈平衡→微网格改用 ρ=1；交接包⑭已发：λ×ρ 2×2 微网格 ρ 最后检验，代码 `30ed45d`）
...
| M9 | 创新点 1 步 C（ρ 不平衡+时序鲁棒）+ 消融终表 10000ep | ◐ | ⑬ 已收：ρ∈{None,10,1,0.1} Acc 全持平、截断 ΔAcc 全 CI 含 0（无显著 ρ 效应）。机制诊断 **ρ=10≈平衡**(col_residual≈0.008)→ **微网格改用 ρ=1** 做真实补偿检验（交接包⑭，λ×ρ 2×2，代码 `30ed45d`）。预期冻结 ρ=None；M9 10000ep 终表待微网格全冻 |
...
## 云端待执行（交接包队列）
- **交接包⑭ M9pre-I1-microgrid（已发，待 Cursor 执行）**：`handoff/2026-09-07_M9pre-I1-microgrid-rho1.md`，代码 `30ed45d`。λ×ρ 2×2 微网格（λ∈{0.3,1}×ρ∈{None,**1**}，ρ=1 为真实不平衡臂；ρ=10 经机制诊断≈平衡已弃）；复用 I1B_lam03/I1C_rho1/I1B_lam1，仅新训 (1,1)；四格同 2500 配对 dev 复评，报 Acc/OS/三截断 ΔAcc/DiD/stage_mass 放松+分布。ρ 最后一次检验：交互明确才议不平衡主线，否则冻结 ε0.1/λ0.3/ρ=None。~8.5h。GPU **空闲**。
- 已收并登账：⑬ M9pre-I1-stepC（ρ扫描+截断，无显著 ρ 效应）、⑩ M8-I1B（λ*=0.3）、⑫ M8pre-I1-probe、⑪ M7-I1A-recheck、⑨ M7-I1A、⑧ M6-3b、⑦ M5-3b、⑥ M4-3a-final、⑤ M3-3a-dev、④ seed2more（B0=57.01±1.64）、③ multiseed、② 历史单跑。
- **当前连云**：Host `gpuhome` → `sc01-ssh.gpuhome.cc` **Port 30448**。GPU **空闲**。

## 已回收待登账
-（空；⑬ 已入 ledger。建议 ρ*=1，截断 ΔAcc CI 均含 0）
...
7. **✅ 微网格交接包⑭已发（本会话）**：ρ=10 机制诊断≈平衡→改用 ρ=1 真实补偿检验；λ×ρ 2×2（仅新训 (1,1)），代码 `30ed45d`（robust_eval 加 OS+stage_mass 放松/分布+per_episode，163/163）。待 Cursor 执行。
8. **待收包⑭后**：算 I_Acc 交互 + 三截断 DiD（配对 CI）+ stage_mass 机制核查 → 冻结 ρ*（预期 None）→ M9 消融终表（10000ep ①B0/②OT平衡/③+λ0.3/④+ρ* + 截断 + 1 种子复跑）；创新点 2/4a 手册已入库待创新点 1 主线定档。
