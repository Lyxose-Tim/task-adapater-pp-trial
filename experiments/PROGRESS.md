# PROGRESS — 跨会话进度
最后更新：2026-09-03（M6 收官登账：λ 曲线成型，λ∈[1,3] 无损平台 OS~0.33、λ=5 Acc降；**创新点 3 闭环**；下一步创新点 1）
...
| M7+ | 创新点 1 起（OT 软阶段分配重写式16），见《后续创新点方案设计_v1》 | ☐ | 前置：用户产出《创新点1_实验执行清单》放 docs/ |
...
## 云端待执行（交接包队列）
-（空——创新点 3 全部收包登账）。**下一个交接包 = 创新点 1（M7），前置待用户产出《创新点1_实验执行清单》。**
- 已收并登账：⑧ M6-3b（λ=2/3）、⑦ M5-3b（λ=1/5）、⑥ M4-3a-final、⑤ M3-3a-dev、④ seed2more（B0=57.01±1.64）、③ multiseed、② 历史单跑。
...
## 下一步
1. **✅ 创新点 3 闭环（本会话）**：M6 五点 λ 曲线成型（`experiments/figures/3b_lambda_curve.png`），消融表入 ledger；3b 结论=顺序正则可控增强 OS，λ∈[1,3] 无损平台（主推荐 λ=3），λ=5 过正则。3a+3b 完整叙事：诊断弱顺序敏感（归因式16）→ 显式监督可控强化。
2. **进创新点 1（M7）**：OT 软阶段分配重写式(16)——正是 3a/3b 的机制靶点。**前置：用户在方案对话产出《创新点1_实验执行清单》放 docs/**（CLAUDE.md §3；B0 已定标满足触发条件）。届时 Claude 按清单写 OT 代码（fsar/ot.py 已就绪）+ 单测（U1 排列不变/U2 极限/U3 梯度）+ 交接包，Cursor 跑云端。
3. 可选：3b 补 λ=4 精定平台→跌落转折（非必须，曲线已清晰）；论文图表冻结。
