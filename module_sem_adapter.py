...
# 坑 B：原为作者机器的绝对路径；改为环境变量优先、仓库相对路径兜底。
import os as _os
CLIP_VIT_B16_PATH = _os.environ.get(
    'CLIP_VIT_B16_PATH',
    _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'dataset', 'checkpoints', 'ViT-B-16.pt'),
)
CLIP_VIT_L14_PATH = _os.environ.get('CLIP_VIT_L14_PATH')
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor,use_mask=True):
        # 原为硬编码 dtype=torch.float16（假设 GPU fp16）；改为匹配 query dtype，
        # GPU(fp16) 下数值不变、CPU(fp32) 单测可跑。掩码位置不变，不影响 B0。
        self.attn_mask = self.attn_mask.to(device=x.device,dtype=x.dtype) if self.attn_mask is not None else None
...
def clip_encode_text_adapter(adapter_layer, checkpoint_path=None):
    ...
    # checkpoint_path 优先（fsar 显式传入），否则回退 CLIP_VIT_B16_PATH 环境变量
    checkpoint = torch.jit.load(checkpoint_path or CLIP_VIT_B16_PATH, map_location='cpu')
