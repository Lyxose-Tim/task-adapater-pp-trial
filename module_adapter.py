...
def clip_vit_base_patch16_adapter(checkpoint_path=None, **kwargs):
    ...
    # checkpoint_path 优先（fsar 工具层显式传入），否则回退 CLIP_VIT_B16_PATH 环境变量
    _path = checkpoint_path or CLIP_VIT_B16_PATH
    assert _path is not None, \
        'Set CLIP_VIT_B16_PATH env var or pass checkpoint_path'
    checkpoint = torch.jit.load(_path, map_location='cpu')
